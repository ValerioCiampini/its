...
key = "itscybersecurity" # chiave AES

messaggio = "Messaggio segreto 123" # messaggio da cifrare

print("\nChiave AES:", key, "\n\nMessaggio da cifrare:", messaggio, "\n")

# cifro il messaggio con AES
messaggio_cifrato = encrypt(str(messaggio), key)

print("Messaggio cifrato con AES:", messaggio_cifrato, "\n")

# estraggo modulo ed esponente pubblico (3) dalla chiave pubblica
#openssl rsa -in FApubkey.pem -pubin -noout -text
...
key = int(key.encode("utf-8").hex(), 16) # converto la chiave in hex per cifrarla in RSA

print("Chiave in hex (da stringa a bytes e da bytes a hex):", key, "\n")

key_cifrata = pow(key, 3, modulo) # cifro il messaggio con esponente pubblico (3) e il modulo 

print("Chiave cifrata pow(chiave, esponente_pubblico (3), modulo):", key_cifrata, "\n")

# estraggo esponente privato dalla chiave privata
#openssl rsa -in FAprivkey.pem -noout -text
...
key = pow(int(key_cifrata), priv_exponent, modulo) # decifro la chiave AES con esponente privato e modulo


# converisone da hex ad ascii fatta da chatgpt non spiegata dal prof
length = (key.bit_length() + 7) // 8
msg_bytes = key.to_bytes(length, "big")
key = msg_bytes.decode("utf-8")


print("Chiave AES decifrata:", key, "\n")

# ora che ho la chiave decifro il messaggio con AES 
messaggio_decifrato = decrypt(messaggio_cifrato, key)

print("Messaggio decifrato da AES:", messaggio_decifrato, "\n")

print("Il messaggio originale è uguale al messaggio decifrato ?", messaggio == messaggio_decifrato, "\n")
